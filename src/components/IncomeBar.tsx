...
export const IncomeBar: React.FC<Props> = ({ income, surplus }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={{
        position: "absolute", bottom: 160, left: 40, width: 500, opacity,
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
          <div>
            ...
            <div style={{ color: "#4ADE80", fontSize: 22, fontWeight: 800 }}>{fmt(income)}</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ color: "#888", fontSize: 10, letterSpacing: 2, textTransform: "uppercase" }}>Surplus</div>
            <div style={{ color: "#60A5FA", fontSize: 22, fontWeight: 800 }}>{fmt(surplus)}</div>
          </div>
        </div>
        <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 6, height: 10, overflow: "hidden" }}>
          <div style={{ width: `${barW * 100}%`, height: "100%", background: "linear-gradient(90deg, #4ADE80, #60A5FA)", borderRadius: 6, transition: "width 0.1s" }} />
        </div>
        <div style={{ color: "#F59E0B", fontSize: 12, fontWeight: 700, marginTop: 6 }}>
          {Math.round(pct * 100)}% savings rate
        </div>
      </div>
    </AbsoluteFill>
  );
